...
namespace ORB {
                                        ...
                                        std::vector<cv::DMatch>& good_matches)
    {
        ...
        for (size_t i=0; i<matches.size(); i++) {
            int bestIdx1 = matches[i].queryIdx;
            int bestIdx2 = matches[i].trainIdx;
            double dist = matches[i].distance;
            if (dist > TH_LOW) continue;
            float rot = keypoints_1[bestIdx1].angle - keypoints_2[bestIdx2].angle;
        ...
        for (int i=0; i<HISTO_LENGTH; i++) {
            if (i == ind1 || i == ind2 || i == ind3) 
            {
                ...
                {
                    int goodIdx = rotHist[i][j];
                    good_matches.push_back(matches[goodIdx]);
                }
            }
        }
